...
class AgentC2(CRobLinkAngs):
    ...
    def write(self):
        ...
        mapc = self.mapc
        matrix = [[' '  for x in range(49)]for y in range(21)]
        refX, refY = list(mapc.keys())[0]
        cL,cC = 10,24
        for xp, yp in mapc.keys():
            pL = (refY-yp) + cL
            pC = (xp-refX) + cC
            for xn, yn in mapc[(xp,yp)][0]:
                nL = (yn-refY) + cL
                nC = (xp-refX) + cC
                if xn > xp and yn == yp: # to the right
                    l = pL
                    c = pC+1
                    matrix[l][c] = "-"
                elif xn < xp and yn == yp: # to the left
                    l = pL
                    c = pC-1
                    matrix[l][c] = "-"
                elif xn == xp and yn > yp: # upwards
                    l = pL-1
                    c = pC
                    matrix[l][c] = "|"
                elif xn == xp and yn < yp: # downwards
                    l = pL+1
                    c = pC
                    matrix[l][c] = "|"
                elif xn > xp and yn > yp: # diagonal foward-up
                    l = pL-1
                    c = pC+1
                    matrix[l][c] = "/"
                elif xn > xp and yn < yp: # diagonal foward-down
                    l = pL+1
                    c = pC+1
                    matrix[l][c] = "\\"
                elif xn < xp and yn > yp: # diagonal backwards-up
                    l = pL-1
                    c = pC-1
                    matrix[l][c] = "\\"
                elif xn < xp and yn < yp: # diagonal backwards-down
                    l = pL+1
                    c = pC-1
                    matrix[l][c] = "/"
                else:
